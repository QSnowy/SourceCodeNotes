...
/// RunLoop 一次运行结果
enum {
    kCFRunLoopRunFinished = 1,  /* 运行结束 */
    kCFRunLoopRunStopped = 2,   /* 运行停止 */
    kCFRunLoopRunTimedOut = 3,  /* 运行超时 */
...
typedef CF_OPTIONS(CFOptionFlags, CFRunLoopActivity) {
    ...
    kCFRunLoopBeforeTimers = (1UL << 1),    /* 定时之前？ */
    kCFRunLoopBeforeSources = (1UL << 2),   /* Source之前 */
...
// RunLoopSource结构体中的 SourceContext
// 包含义很多函数指针，
// source0可以定时和取消，没有获取port的功能；source1不能定时也不能取消，但是可以获取port。
typedef struct {
    CFIndex	version;    /* 版本号?? */
    void *	info;   /* 执行、取消等函数需要的Info */
    const void *(*retain)(const void *info);        /* retain函数 ?? */
    void	(*release)(const void *info);           /* release函数 ??*/
    CFStringRef	(*copyDescription)(const void *info);   /* copyDesc函数指针 */
    Boolean	(*equal)(const void *info1, const void *info2); /* equal函数指针 */
    CFHashCode	(*hash)(const void *info);  /* hash算法 */
    void	(*schedule)(void *info, CFRunLoopRef rl, CFStringRef mode); /* 计划安排函数 */
    void	(*cancel)(void *info, CFRunLoopRef rl, CFStringRef mode);   /* 取消函数*/
    void	(*perform)(void *info);     /* 执行函数 */
} CFRunLoopSourceContext;

// RunLoopSource结构体中的 SourceContext1
// 里面有获取port的函数，source1的执行与port有关系。
typedef struct {
    CFIndex	version;
    void *	info;
...
#if (TARGET_OS_MAC && !(TARGET_OS_EMBEDDED || TARGET_OS_IPHONE)) || (TARGET_OS_EMBEDDED || TARGET_OS_IPHONE)
    mach_port_t	(*getPort)(void *info)      /* 获取port */
    void *	(*perform)(void *msg, CFIndex size, CFAllocatorRef allocator, void *info); /* 执行 */
#else
...
// RunLoopTimerContext的定义，跟ObserverContext数据构成是一样的。
typedef struct {
    CFIndex	version;
    void *	info;
    const void *(*retain)(const void *info);
